...
import 'package:motel/screens/date_picker/classes/event.dart';
import 'package:motel/screens/date_picker/classes/event_list.dart';
import 'package:motel/screens/date_picker/src/default_styles.dart';
import 'package:motel/screens/date_picker/src/calendar_header.dart';
import 'package:motel/screens/date_picker/src/weekday_row.dart';
import 'package:intl/date_symbol_data_local.dart';
import 'package:intl/intl.dart' show DateFormat;
export 'package:motel/screens/date_picker/classes/event_list.dart';
...
class CalendarCarousel<T extends EventInterface> extends StatefulWidget {
  ...
  CalendarCarousel({
    ...
    this.daysTextStyle=const TextStyle(color: Color(0xFF3B3E5D)),
    ...
    this.todayTextStyle=const TextStyle(color: Color(0xFF3B3E5D)),
    ...
    this.onDayPressed,
    this.weekdayTextStyle=const TextStyle(color: Color(0xFF3B3E5D)),
    this.iconColor = Colors.blueAccent,
    this.headerTextStyle=const TextStyle(color: Color(0xFF3B3E5D),fontSize: 18,fontWeight: FontWeight.w600),
    this.headerText,
    this.weekendTextStyle=const TextStyle(color: Color(0xFF3B3E5D)),
    this.markedDatesMap,
    ...
    this.markedDateCustomTextStyle=const TextStyle(color: Colors.red),
    ...
    this.pageScrollPhysics = const NeverScrollableScrollPhysics(), this.monthToShow,
...
class _CalendarState<T extends EventInterface> extends State<CalendarCarousel<T>> {
  ...
  DateTime _monthToShow=DateTime.now();
  ...
  initState() {
    ...
    if(widget.monthToShow !=null)
      _monthToShow=widget.monthToShow;
  ...
  Widget build(BuildContext context) {
    if (widget.resetDate != null) {
      setState(() {
        _selectedDate = [widget.resetDate];
      });
    }
    return Container(
      width: widget.width,
  ...
  Widget getDefaultDayContainer(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isSelectedDay?Color(0xFF00AEEF):Colors.white,
        borderRadius: BorderRadius.circular(8),
        border: Border.all(width: 1, color: Color.fromRGBO(247, 249, 252, 1))
      ),
      child: Row(
              ...
              child: GridView.count(
                ...
                children: List.generate(totalItemCount,
                  ...
                  for(int i=0;i<_selectedDate.length;i++){
                    if(isSelectedDay)break;
                    isSelectedDay = _selectedDate != null &&
                        _selectedDate[i].year == year &&
                        _selectedDate[i].month == month &&
                        _selectedDate[i].day == index + 1 - _startWeekday;
                  }
              ...
              child: Container(
                ...
                child: GridView.count(
                  ...
                  children: List.generate(weekDays.length, (index) { /// last day of month + weekday
                    ...
                    for(int i=0;i<_selectedDate.length;i++){
                      if(isSelectedDay)break;
                      isSelectedDay = this._selectedDate != null &&
                          this._selectedDate[i].year == weekDays[index].year &&
                          this._selectedDate[i].month == weekDays[index].month &&
                          this._selectedDate[i].day == weekDays[index].day;
                    }
  ...
  void _onDayPressed(DateTime picked) {
    ...
    if(widget.chooseMultiple){
      shouldAdd=_selectedDate.length<2;
    }

    for(int i=0;i<_selectedDate.length;i++){
      if(_selectedDate[i].compareTo(picked)==0)
        shouldAdd=false;
    }
    if(shouldAdd){
      setState(() {
  ...
  Future<Null> _selectDateFromPicker() async {
    ...
    if (selected != null) {
      ...
      if(_selectedDate.length<2)
        setState(() {
          _selectedDate.add(selected);
        });
      if (widget.onDayPressed != null)
        widget.onDayPressed(
          selected,
